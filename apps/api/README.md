...
- File Storage: S3-compatible object storage (MinIO in development, AWS S3 in production) via `@aws-sdk/client-s3`
- Authentication and Security:
  ...
  - Cookie parser via `@fastify/cookie`
...
- `GET /api/v1/sales-visits`: Query client visit records with search and pagination
...
- `GET /api/v1/enquiries`: List enquiries with stage filtering
...
- `GET /api/v1/engineering/documents`: Query engineering drawings and specifications
...
- `GET /api/v1/procurement/purchase-orders`: List purchase orders with vendor details
- `POST /api/v1/procurement/purchase-orders`: Issue purchase order with line items
- `PATCH /api/v1/procurement/purchase-orders/:id/status`: Update procurement milestone

### Manufacturing (`/api/v1/manufacturing`)
- `GET /api/v1/manufacturing/batches`: Production batch tracker
...
The API requires the following environment variables (defined in root `.env` or container environment):
...
Run scripts from the repository root using pnpm workspace filters or directly inside `apps/api`:
